...
using PromotorEntity = WePlayRises.Crowdpromotion.Domain.Model.Promotor;
...
public class SolicitarInscripcionCommandHandlerTests
{
    ...
    public async Task Handle_ValidCommand_ReturnsCreatedResult()
    {
        ...
        result.Data!.ProgramaTitulo.Should().Be("Test Programa");
        result.Messages.Should().Contain(m => m.HttpStatusCode == HttpStatusCode.Created);
    }
    ...
    {
        // Arrange
        var command = new SolicitarInscripcionCommand { ProgramaId = Guid.Empty, UserId = "" };
    ...
    public async Task Handle_PromotorNotFound_ReturnsNotFound()
    {
        ...
        _serviceMock.Setup(s => s.GetPromotorByUserIdAsync(command.UserId, It.IsAny<CancellationToken>()))
            .ReturnsAsync((PromotorEntity?)null);
    ...
    public async Task Handle_ProgramaNotFound_ReturnsNotFound()
    {
        ...
        _serviceMock.Setup(s => s.GetProgramaByIdAsync(It.IsAny<PromoProgramaId>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((PromoPrograma?)null);
    ...
    public async Task Handle_InscripcionAlreadyExists_ReturnsBadRequest()
    {
        ...
        var promotor = InscripcionTestData.CreateValidPromotor();
        var programa = InscripcionTestData.CreateValidPrograma();
        var existing = InscripcionTestData.CreatePendienteInscripcion();
        ...
        // Assert
        result.IsSuccess.Should().BeFalse();
        result.Messages.Should().Contain(m => m.ErrorCode == ServiceResponseMessageType.BusinessRule_InscripcionAlreadyExists);
    }
    ...
    public async Task Handle_InscripcionBloqueada_ReturnsForbidden()
    {
        ...
        var promotor = InscripcionTestData.CreateValidPromotor();
        var programa = InscripcionTestData.CreateValidPrograma();
        var blocked = InscripcionTestData.CreateBloqueadaInscripcion();
        ...
        // Assert
        result.IsSuccess.Should().BeFalse();
        result.Messages.Should().Contain(m => m.ErrorCode == ServiceResponseMessageType.BusinessRule_InscripcionBloqueada);
    }
